...
(function(global, factory) {
	typeof exports === "object" && typeof module !== "undefined" ? module.exports = factory(require("@floating-ui/dom")) : typeof define === "function" && define.amd ? define(["@floating-ui/dom"], factory) : (global = typeof globalThis !== "undefined" ? globalThis : global || self, global.bootstrap = factory(global.FloatingUIDOM));
})(this, function(_floating_ui_dom) {
	...
	*/
	...
	const EVENT_KEY$43 = `.bs.accordion`;
	...
	*/
	...
	const EVENT_KEY$41 = `.bs.list-box`;
	...
	const SELECTOR_DATA_LIST_BOX = "[data-bs-list-box]";
	const SELECTOR_COUNTER = "[data-bs-list-box-counter]";
	...
	const SELECTOR_SEARCH$3 = "[data-bs-list-box-search]";
	const SELECTOR_SECTION$2 = ".list-box-section";
	const SELECTOR_SECTION_LABEL = ".list-box-section-label";
	const SELECTOR_SECTION_TOGGLE = "[data-bs-section-toggle]";
	const SELECTOR_SELECT_ALL$2 = "[data-bs-select-all]";
	const ATTRIBUTE_INDICATOR = "data-bs-indicator";
	const ATTRIBUTE_SECTION_TOGGLE = "data-bs-section-toggle";
	...
	*/
	var ListBox = class ListBox extends BaseComponent {
		...
		_resolveCounter() {
			...
			counter.setAttribute("data-bs-list-box-counter", "");
		...
		_resolveSearch() {
			...
			search.setAttribute("data-bs-list-box-search", "");
		...
		_renderOption(item) {
			...
			option.dataset.bsValue = item.value;
			if (item.disabled) {
		...
		_decorateSection(section) {
			...
			label.dataset.bsValue = label.id;
		...
		_optionValue(option) {
			return option.dataset.bsValue ?? option.textContent?.trim() ?? "";
		}
		_optionText(option) {
	...
	*/
	const createAnchoredPosition = (anchor, content) => {
		...
		const update = async () => {
			...
			const { x, y } = await (0, _floating_ui_dom.computePosition)(anchor, content, {
				middleware: [
					(0, _floating_ui_dom.offset)({ mainAxis: 2 }),
					(0, _floating_ui_dom.flip)(),
					(0, _floating_ui_dom.shift)({ boundary: "clippingAncestors" })
				],
		...
		update();
		const cleanup = (0, _floating_ui_dom.autoUpdate)(anchor, content, update);
		return {
			update,
			destroy() {
				disposed = true;
	...
	*/
	...
	const EVENT_KEY$40 = `.bs.focustrap`;
	...
	*/
	var Popup = class extends Config {
		...
		_startPositioning() {
			this._cleanupAutoUpdate = (0, _floating_ui_dom.autoUpdate)(this._anchor, this._content, () => this._updatePosition());
		}
		_stopPositioning() {
		...
		_updatePosition() {
			...
			const middleware = [
				(0, _floating_ui_dom.offset)({
					crossAxis: skidding,
					mainAxis: distance
				}),
				(0, _floating_ui_dom.flip)(this._config.fallbackPlacements ? { fallbackPlacements: this._config.fallbackPlacements } : {}),
				(0, _floating_ui_dom.shift)()
			];
			(0, _floating_ui_dom.computePosition)(this._anchor, this._content, {
				middleware,
	...
	*/
	...
	const EVENT_LIST_BOX_CHANGE$1 = "change.bs.list-box";
	const EVENT_LIST_BOX_DESELECTED = "deselected.bs.list-box";
	const EVENT_LIST_BOX_SELECTED = "selected.bs.list-box";
	const EVENT_LIST_BOX_SELECTION_LIMIT = "selectionLimit.bs.list-box";
	...
	*/
	...
	const DATA_KEY$2 = "bs.autocomplete";
	...
	const SELECTOR_DATA_AUTOCOMPLETE = "[data-bs-autocomplete]:not(.disabled)";
	...
	*/
	...
	const EVENT_KEY$37 = `.bs.button`;
	const DATA_API_KEY$32 = ".data-api";
	const CLASS_NAME_ACTIVE$8 = "active";
	const SELECTOR_DATA_TOGGLE$13 = "[data-bs-toggle=\"button\"]";
	...
	*/
	...
	const EVENT_KEY$36 = `.bs.calendar`;
	...
	*/
    ...
    `;
		...
		_navIcon(name) {
			return sanitizeByConfig(this._config[isRTL(this._element) ? {
	...
	*/
	...
	const EVENT_KEY$35 = `.bs.carousel`;
	...
	*/
	var Carousel = class Carousel extends BaseComponent {
		...
		_scrollDelta(element) {
			...
			return isRTL(this._element) ? rect.right - (viewportRect.right - padStart) : rect.left - (viewportRect.left + padStart);
		}
		...
		_loopDirection(isNext) {
			if (isRTL(this._element)) return isNext ? DIRECTION_RIGHT : DIRECTION_LEFT;
	...
	*/
	...
	const EVENT_KEY$33 = `.bs.chip-set`;
	...
	*/
	...
	const EVENT_KEY$32 = `.bs.chip-input`;
	...
	var ChipInput = class extends ChipSet {
		...
		_handleInputKeydown(event) {
			...
			switch (key) {
				...
				case "ArrowLeft":
				case "ArrowRight":
					if (key === (isRTL(this._element) ? "ArrowRight" : "ArrowLeft") && this._input.selectionStart === 0 && this._input.selectionEnd === 0) {
						event.preventDefault();
						const chips = this._getChipElements();
						if (chips.length > 0) chips[chips.length - 1].focus();
					}
	...
	*/
	...
	const EVENT_KEY$31 = `.bs.collapse`;
	...
	const SELECTOR_DATA_TOGGLE$12 = "[data-bs-toggle=\"collapse\"]";
	...
	*/
	...
	const EVENT_LIST_BOX = ".bs.list-box";
	...
	const SELECTOR_DATA_TOGGLE$11 = "[data-bs-toggle=\"combobox\"]";
	...
	const SELECTOR_SEARCH$2 = "[data-bs-list-box-search]";
	...
	*/
	var Menu = class Menu extends BaseComponent {
		...
		constructor(element, config) {
			if (typeof _floating_ui_dom.computePosition === "undefined") throw new TypeError("CoreUI's menus require Floating UI (https://floating-ui.com)");
			super(element, config);
			this._floatingCleanup = null;
		...
		_getConfig(config) {
			...
			if (typeof config.reference === "object" && !isElement(config.reference) && typeof config.reference.getBoundingClientRect !== "function") throw new TypeError(`${NAME$37.toUpperCase()}: Option "reference" provided type "object" without a required "getBoundingClientRect" method.`);
			return config;
		}
		_createFloating() {
			...
			this._floatingCleanup = (0, _floating_ui_dom.autoUpdate)(referenceElement, this._menu, () => this._updateFloatingPosition(referenceElement));
		}
		...
		_getReferenceElement() {
			...
			if (isElement(reference)) return getElement(reference);
		...
		_resolveLogicalPlacement(placement, element = this._element) {
			if (isRTL(element)) return placement.replace(/^start(?=-|$)/, "right").replace(/^end(?=-|$)/, "left");
		...
		_getFloatingMiddleware() {
			...
			return [
				(0, _floating_ui_dom.offset)(typeof offsetValue === "function" ? offsetValue : toFloatingOffset(offsetValue)),
				(0, _floating_ui_dom.flip)({
		...
		async _applyFloatingPosition(reference, floating, placement, middleware, strategy = "absolute") {
			...
			const { x, y, placement: finalPlacement } = await (0, _floating_ui_dom.computePosition)(reference, floating, {
				placement,
				middleware,
				strategy
			});
		...
		_createSubmenuFloating(trigger, submenu, submenuWrapper) {
			...
			const middleware = [
				...
				(0, _floating_ui_dom.flip)({ fallbackPlacements: [
			...
			updatePosition();
			return (0, _floating_ui_dom.autoUpdate)(referenceElement, submenu, updatePosition);
		}
		...
		_isMovingTowardSubmenu(event, submenu) {
			...
			const targetX = isRTL(submenu) ? submenuRect.right : submenuRect.left;
			const topCorner = {
				x: targetX,
				y: submenuRect.top
			};
	...
	*/
	...
	const EVENT_KEY$28 = `.bs.context-menu`;
	...
	const SELECTOR_DATA_TOGGLE$10 = "[data-bs-toggle=\"context-menu\"]";
	const Default$35 = {
		...Menu.Default,
		reference: "pointer",
		strategy: "fixed"
	...
	*/
	...
	const SELECTOR_FORM_VALIDATE = "[data-bs-validate]";
	const SELECTOR_FORM_VALIDATE_VALID$1 = "[data-bs-validate~=\"valid\"]";
	...
	* generated, so a bare `<div data-bs-date-picker>` keeps working.
	...
	*/
	...
	const EVENT_KEY$27 = `.bs.date-picker`;
	...
	const SELECTOR_DATA_DATE_PICKER = "[data-bs-date-picker]";
	const SELECTOR_ROLE_CLEANER = "[data-bs-picker-cleaner]";
	const SELECTOR_ROLE_FIELD = "[data-bs-picker-field]";
	const SELECTOR_ROLE_TOGGLE = "[data-bs-picker-toggle]";
	...
	*/
	...
	const EVENT_KEY$26 = `.bs.date-range-input`;
	...
	const ATTRIBUTE_ROLE_END = "data-bs-range-end";
	const ATTRIBUTE_ROLE_SEPARATOR = "data-bs-range-separator";
	const ATTRIBUTE_ROLE_START = "data-bs-range-start";
	const SELECTOR_DATA_DATE_RANGE_INPUT = "[data-bs-date-range-input]";
	...
	*/
	var DateRangeInput = class DateRangeInput extends BaseComponent {
		...
		_createSeparator() {
			...
			separator.innerHTML = sanitizeByConfig(isRTL(this._element) ? this._config.separatorIconRtl : this._config.separatorIcon, this._config);
			return separator;
		}
		...
		_addEventListeners() {
			...
			EventHandler.on(this._element, EVENT_KEYDOWN$6, (event) => {
				...
				const forward = event.key === (isRTL(this._element) ? ARROW_LEFT_KEY$4 : ARROW_RIGHT_KEY$4);
	...
	*/
	...
	const EVENT_KEY$25 = `.bs.date-range-picker`;
	...
	const SELECTOR_DATA_DATE_RANGE_PICKER = "[data-bs-date-range-picker]";
	...
	*/
	...
	const EVENT_KEY$24 = `.bs.date-time-picker`;
	...
	const SELECTOR_DATA_DATE_TIME_PICKER = "[data-bs-date-time-picker]";
	const Default$28 = {
		allowList: SVGAllowlist,
	...
	*/
	...
	const SELECTOR_DATA_TOGGLE$9 = "[data-bs-toggle=\"dialog\"]";
	const Default$27 = {
		backdrop: true,
		keyboard: true,
		modal: true
	...
	*/
	...
	const SELECTOR_DATA_TOGGLE$8 = "[data-bs-toggle=\"drawer\"]";
	...
	*/
	...
	const EVENT_KEY$21 = `.bs.dropdown`;
	...
	*/
	...
	const EVENT_KEY$20 = `.bs.loading-button`;
	...
	const SELECTOR_DATA_TOGGLE$7 = "[data-bs-toggle=\"loading-button\"]";
	const Default$24 = {
	...
	*/
	...
	const SELECTOR_DATA_TOGGLE$6 = "[data-bs-toggle=\"modal\"]";
	const Default$23 = {
		backdrop: true,
		keyboard: true,
		modal: true
	...
	*/
	...
	const SELECTOR_SELECT_ALL$1 = "[data-bs-select-all]";
	...
	var MultiSelect = class MultiSelect extends ComboboxBase {
		...
		_focusListBox() {
			...
			if (this._listBox.getActive() === null && !this._searchElement?.value) {
				...
				if (selected?.dataset.bsValue) this._listBox.setActive(selected.dataset.bsValue);
			}
		...
		selectFiltered() {
			...
			for (const item of items) {
				...
				const value = String(item.dataset.bsValue);
			...
			EventHandler.on(this._wrapperElement, EVENT_KEYDOWN$4, (event) => {
				...
				if (this._isShown() && event.target === this._searchElement) {
					...
					if (event.key === ENTER_KEY) {
						...
						if (stop?.dataset.bsValue) {
							event.preventDefault();
							this._listBox?.toggle(stop.dataset.bsValue);
						}
						return;
					}
				}
		...
		_buildMenuHeader(listBoxDiv) {
			...
			if (hasHeaderTemplate) {
				...
				selectAllButton.setAttribute("data-bs-select-all", "");
		...
		_getOptionElement(value) {
			return SelectorEngine.findOne(`[data-bs-value="${CSS.escape(value)}"]`, this._optionsElement);
		}
		_getDisplayedItems() {
	...
	*/
	...
	const EVENT_KEY$17 = `.bs.navigation`;
	...
	*/
	...
	const EVENT_KEY$16 = `.bs.offcanvas`;
	...
	const SELECTOR_DATA_TOGGLE$5 = "[data-bs-toggle=\"offcanvas\"]";
	...
	*/
	...
	const EVENT_KEY$15 = `.bs.otp-input`;
		...
		_addEventListeners() {
			...
			EventHandler.on(this._element, EVENT_KEYDOWN$3, SELECTOR_FORM_OTP_CONTROL, (event) => {
				...
				if (key === ARROW_RIGHT_KEY$2 || key === ARROW_LEFT_KEY$2) {
					const shouldMoveNext = key === ARROW_RIGHT_KEY$2 !== isRTL(this._element);
	...
	*/
	...
	const EVENT_KEY$14 = `.bs.number-input`;
	...
	const SELECTOR_DATA_NUMBER_INPUT = "[data-bs-number-input]";
	...
	*/
	...
	const EVENT_KEY$13 = `.bs.password-input`;
	...
	const SELECTOR_DATA_PASSWORD_INPUT = "[data-bs-password-input]";
	const Default$17 = {
		allowList: SVGAllowlist,
	...
	*/
	...
	const EVENT_KEY$12 = `.bs.password-strength`;
	...
	const SELECTOR_DATA_PASSWORD_STRENGTH = "[data-bs-password-strength]";
	const SELECTOR_PASSWORD = "input[type=\"password\"], input[data-bs-password-input]";
	const Default$16 = {
		busyLabel: "Checking…",
		debounce: 200,
		input: null,
	...
	*/
	...
	const SELECTOR_DATA_TOGGLE$4 = "[data-bs-toggle=\"tooltip\"]";
	...
	*/
	...
	const SELECTOR_DATA_TOGGLE$3 = "[data-bs-toggle=\"popover\"]";
	...
	*/
	...
	const EVENT_DOM_CONTENT_LOADED$1 = `DOMContentLoaded.bs.progress.data-api`;
	...
	const PROPERTY_SEGMENTS = "--bs-progress-segments";
	const PROPERTY_SEGMENTS_FIT = "--bs-progress-segments-fit";
	const PROPERTY_SEGMENT_RADIUS_FIT = "--bs-progress-segment-radius-fit";
	const PROPERTY_SEGMENTS_FILLED = "--bs-progress-segments-filled";
	const PROPERTY_SEGMENT_GAP = "--bs-progress-segment-gap";
	const PROPERTY_SEGMENT_MIN_WIDTH = "--bs-progress-segment-min-width";
	const PROPERTY_SEGMENT_BORDER_RADIUS = "--bs-progress-segment-border-radius";
	/**
	* Class definition
	*/
	...
	*/
	...
	const EVENT_KEY$10 = `.bs.range-slider`;
	...
	const SELECTOR_DATA_RANGE_SLIDER = "[data-bs-range-slider]";
	...
	*/
	var RangeSlider = class RangeSlider extends BaseComponent {
		...
		_updateGradient() {
			...
			if (this._currentValue.length === 1) this._sliderTrack.style.setProperty("--bs-range-slider-track-from-edge", "0");
			else this._sliderTrack.style.setProperty("--bs-range-slider-track-from", ratio(min));
			this._sliderTrack.style.setProperty("--bs-range-slider-track-to", ratio(max));
		}
	...
	*/
	...
	const SELECTOR_DATA_RATING = "[data-bs-rating]";
	...
	*/
	...
	const EVENT_KEY$8 = `.bs.scrollspy`;
	...
	*/
	...
	const EVENT_KEY$7 = `.bs.search-button`;
	...
	*/
	...
	const SELECTOR_DATA_STEPPER = "[data-bs-stepper]";
	const SELECTOR_FORM_VALIDATE_VALID = "[data-bs-validate~=\"valid\"]";
	...
	*/
	...
	const EVENT_KEY$3 = `.bs.time-picker`;
	...
	const SELECTOR_DATA_TIME_PICKER = "[data-bs-time-picker]";
	const Default$3 = {
		allowList: SVGAllowlist,
	...
	*/
	...
	const EVENT_HIDE_TOAST = "hide.bs.toast";
	const EVENT_HIDDEN_TOAST = "hidden.bs.toast";
	...
	const ATTRIBUTE_ID = "data-bs-toast-id";
	const ATTRIBUTE_LIMITED = "data-bs-limited";
	const ATTRIBUTE_STACK_INDEX = "data-bs-stack-index";
	const ATTRIBUTE_STACK_HIDDEN = "data-bs-stack-hidden";
	const ATTRIBUTE_UPDATE_KEY = "data-bs-update-key";
	const PROPERTY_STACK_INDEX = "--bs-toast-stack-index";
	const STACK_VISIBLE = 3;
	const GLIDE_ID = "bs.toaster.glide";
	const PROPERTY_STACK_BEFORE = "--bs-toast-stack-before";
	const PROPERTY_STACK_COUNT = "--bs-toast-stack-count";
	const PROPERTY_STACK_FRONT_HEIGHT = "--bs-toast-stack-front-height";
	const PROPERTY_STACK_HEIGHTS = "--bs-toast-stack-heights";
	const PROPERTY_TOAST_HEIGHT = "--bs-toast-height";
	const PROPERTY_ENTER_TRANSLATE = "--bs-toast-enter-translate";
	const PROPERTY_LEAVE_TRANSLATE = "--bs-toast-leave-translate";
	const EDGES = {
		bottom: "0 calc(100% + var(--bs-toast-container-inset))",
		end: "calc(var(--bs-toast-inline-sign) * (100% + var(--bs-toast-container-inset))) 0",
		start: "calc(var(--bs-toast-inline-sign) * (-100% - var(--bs-toast-container-inset))) 0",
		top: "0 calc(-100% - var(--bs-toast-container-inset))"
	};
	...
	*/
	var Toaster = class Toaster extends BaseComponent {
		...
		_announce(toast) {
			...
			const text = [toast.title, toast.description].map((part) => execute(part, [void 0, this])).map((part) => isElement(part) ? part.textContent : part).filter(Boolean).join(". ");
	...
	*/
	...
	const EVENT_LIST_BOX_CHANGE = "change.bs.list-box";
	const EVENT_LIST_BOX_SEARCH = "search.bs.list-box";
	...
	const SELECTOR_DATA_TRANSFER = "[data-bs-transfer]";
	const SELECTOR_LIST = "[data-bs-transfer-list]";
	const SELECTOR_MOVE = "[data-bs-transfer-move]";
	const SELECTOR_OPTION = ".list-box-option";
	const SELECTOR_OPTIONS = ".list-box-options";
	const SELECTOR_SEARCH = "[data-bs-list-box-search]";
	const SELECTOR_SELECT_ALL = "[data-bs-select-all]";
	...
	*/
	var Transfer = class Transfer extends BaseComponent {
		...
		_createSide(name) {
			const element = SelectorEngine.findOne(`[data-bs-transfer-list="${name}"]`, this._element);
			if (!element) throw new TypeError(`Transfer requires a [data-bs-transfer-list="${name}"] element`);
		...
		_moveKind(button) {
			const kind = button.dataset.bsTransferMove ?? "";
		...
		_sideOf(element) {
			...
			return list ? this._sides[list.dataset.bsTransferList] ?? null : null;
		}
		_reportSearch(event) {
