...
describe('ScrollSpy', () => {
  ...
  const testElementIsActiveAfterScroll = ({ elementSelector, targetSelector, contentEl, scrollSpy, cb }) => {
    ...
    contentEl.addEventListener('activate.coreui.scrollspy', event => {
  ...
  describe('DATA_KEY', () => {
    it('should return plugin data key', () => {
      expect(ScrollSpy.DATA_KEY).toEqual('coreui.scrollspy')
    })
  })
  ...
  describe('constructor', () => {
    ...
    it('should respect threshold option markup', () => {
      fixtureEl.innerHTML = [
        ...
        '   <a class="nav-link active" id="one-link" href="#">One</a>' +
        '</ul>',
        '<div id="content" data-coreui-threshold="0,0.2,1">',
        '  <div id="one-link">test</div>',
        '</div>'
      ].join('')
  ...
  describe('event handler', () => {
    it('should create scrollspy on window load event', () => {
      fixtureEl.innerHTML = [
        '<div id="nav"></div>' +
        '<div id="wrapper" data-coreui-spy="scroll" data-coreui-target="#nav" style="overflow-y: auto"></div>'
      ].join('')
  ...
  describe('SmoothScroll', () => {
    it('should not enable smoothScroll', () => {
      ...
      expect(offSpy).not.toHaveBeenCalledWith(target, 'click.coreui.scrollspy')
      expect(onSpy).not.toHaveBeenCalledWith(target, 'click.coreui.scrollspy')
    })
    ...
    it('should enable smoothScroll', () => {
      ...
      expect(offSpy).toHaveBeenCalledWith(target, 'click.coreui.scrollspy')
      expect(onSpy).toHaveBeenCalledWith(target, 'click.coreui.scrollspy', '[href]', jasmine.any(Function))
    })
