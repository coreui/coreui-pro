...
describe('TimePickerV2', () => {
  ...
  const buildPicker = (config = {}, html = '<div id="picker"></div>') => {
    ...
    const picker = new TimePickerV2(fixtureEl.querySelector('#picker'), { locale: 'en-US', ...config })
    pickers.push(picker)
    return picker
  }
  ...
  describe('time selection', () => {
    it('should update the field and emit timeChange when a cell is clicked', () => {
      ...
      const el = fixtureEl.querySelector('#picker')
      let emitted = null
      el.addEventListener('timeChange.coreui.time-picker-v2', event => {
        emitted = event.time
      })
