...
describe('DateRangePickerV2', () => {
  ...
  const buildPicker = (config = {}, html = '<div id="picker"></div>') => {
    ...
    const picker = new DateRangePickerV2(fixtureEl.querySelector('#picker'), config)
    pickers.push(picker)
    return picker
  }
  ...
  describe('range selection', () => {
    it('should update both fields, emit both events, and close after the end date', () => {
      ...
      const el = fixtureEl.querySelector('#picker')
      const events = []
      el.addEventListener('startDateChange.coreui.date-range-picker-v2', event => events.push(['start', event.date]))
      el.addEventListener('endDateChange.coreui.date-range-picker-v2', event => events.push(['end', event.date]))
  ...
  describe('slot context', () => {
    ...
    it('should set a range through the context and emit both events', () => {
      ...
      const el = fixtureEl.querySelector('#picker')
      const events = []
      el.addEventListener('startDateChange.coreui.date-range-picker-v2', () => events.push('start'))
      el.addEventListener('endDateChange.coreui.date-range-picker-v2', () => events.push('end'))
