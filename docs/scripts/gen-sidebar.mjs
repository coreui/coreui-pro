...
const SRC = join(here, '../../docs-old/data/sidebar.yml')
const REACT_SIDEBAR = join(here, '../../../coreui-react/packages/docs/src/data/sidebar.yml')
const ICONS = join(here, '../../docs-old/layouts/partials/icons')
...
const groups = sidebar.map((group) => {
  ...
  const pages = (group.pages ?? []).map((page) => ({
    ...
    // Carry the Hugo PRO/NEW badge through (engine normalizes single → array).
