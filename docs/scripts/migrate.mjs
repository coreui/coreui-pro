  ...
  const chain = (
    extractRawExamples(expandRanges(body))
      ...
      .replace(/\{\{< example([^>]*)>\}\}\n([\s\S]*?)\n\{\{< \/example >\}\}/g, (_full, attrs, html) => {
        ...
        // `stackblitz_pro="true"` marks a Pro example so its sandbox scaffolds against the
        // Pro packages (a page can mix free and Pro examples).
        const pro = /stackblitz_pro="true"/.test(attrs) ? ' pro' : ''
        return `<Example${pro} lang="html" name="${name}" componentName="${componentName}"${cls ? ` class="${cls}"` : ''} code={${arr}} html={${arr}}${sandboxJs} />`
      })
