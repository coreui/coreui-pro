...
<Example code={`<a href="#" class="d-inline-flex focus-ring py-1 px-2 text-decoration-none border rounded-2" style="--cui-focus-ring-color: rgba(var(--cui-success-rgb), .25)">
    Green focus ring
  </a>`} />
...
By default, there is no `--cui-focus-ring-x`, `--cui-focus-ring-y`, or `--cui-focus-ring-blur`, but we provide CSS variables with fallbacks to initial `0` values. Modify them to change the default appearance.

<Example code={`<a href="#" class="d-inline-flex focus-ring py-1 px-2 text-decoration-none border rounded-2" style="--cui-focus-ring-x: 10px; --cui-focus-ring-y: 10px; --cui-focus-ring-blur: 4px">
