...
`limit` caps how many toasts are visible at once. The newest stay on screen; older ones fade out and remain in the DOM with `inert` and `data-coreui-limited`, then slide back in as the visible ones go away.
