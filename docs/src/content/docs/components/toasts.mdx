...
Toasts you create at runtime — from a form submission, a request that settled, a background job — come from the [Toaster](/components/toaster/): a store keyed by id with `add()`, `update()`, `close()` and `promise()`, its own placed container with persistent live regions, a visible-toasts limit and screen-reader priority. The markup and classes on this page are what it renders.
...
<ScssDocs file="scss/_toasts.scss" capture="toast-tokens" />
