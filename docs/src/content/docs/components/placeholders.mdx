...
<Example code={[`<span class="placeholder col-12"></span>`, ...getData('theme-colors').map((c) => `<span class="placeholder col-12 bg-${c.name}"></span>`)]} />
...
Placeholders use local CSS variables on `.placeholder` and `.placeholder-wave` for real-time customization.

<ScssDocs file="scss/_placeholder.scss" capture="placeholder-tokens" />
