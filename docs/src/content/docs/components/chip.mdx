...
| --- | --- | --- | --- |
| `ariaRemoveLabel` | string | `'Remove'` | Accessible label for the remove button. Unused inside a selectable [chip set](/components/chip-set/#accessibility), where the affordance is hidden from assistive technology. |
...
| `dispose` | Destroys a chip instance and takes back what it added to the chip: the remove button, the check icon, and the classes, role, `tabindex` and ARIA attributes it set. What the markup already had stays. (Removes stored data on the DOM element) |
...
- Each `.chip-remove` button includes an accessible label via `ariaRemoveLabel`. Inside a selectable [chip set](/components/chip-set/#accessibility) the chip is a `role="option"` and the affordance is a `<span aria-hidden="true">` instead, so the label does not apply there.
...
Chips use local CSS variables on `.chip` for enhanced real-time customization.

<ScssDocs file="scss/_chip.scss" capture="chip-tokens" />
...
<ScssDocs file="scss/_legacy.scss" capture="chip-modifiers" />
