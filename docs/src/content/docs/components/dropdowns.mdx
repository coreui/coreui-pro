...
**Built on Menu — and heading for retirement.** Since v6 the Dropdown plugin is a compatibility layer over the new [Menu](/components/menu/) component — same engine, keyboard handling and dismissal, with the v5 markup, classes and `*.coreui.dropdown` events preserved. Existing dropdown markup keeps working unchanged throughout v6, but **Dropdown may be removed in v7** — we recommend using Menu for new work and migrating when convenient.
</Callout>
...
Dropdowns are built on a third party library, [Floating UI](https://floating-ui.com/), which provides dynamic positioning and viewport detection. Be sure to include [floating-ui.core.umd.min.js](https://cdn.jsdelivr.net/npm/@floating-ui/core@1.8.0/dist/floating-ui.core.umd.min.js) and [floating-ui.dom.umd.min.js](https://cdn.jsdelivr.net/npm/@floating-ui/dom@1.8.0/dist/floating-ui.dom.umd.min.js) (in this order) before CoreUI's JavaScript or use `coreui.bundle.min.js` / `coreui.bundle.js` which contains Floating UI. Floating UI isn't used to position dropdowns in navbars though as dynamic positioning isn't required.
...
<Example code={`<nav class="navbar navbar-expand-lg bg-3" data-coreui-theme="dark">
...
<Callout color="info">
**Heads up!** Dropdowns are positioned thanks to Floating UI except when they are contained in a navbar.
</Callout>
...
Place any freeform text within a dropdown menu with text and use [spacing utilities](/utilities/margin/). Note that you'll likely need additional sizing styles to constrain the menu width.

<Example code={`<div class="dropdown-menu p-4 fg-2" style="max-width: 200px;">
    <p>
...
Put a form within a dropdown menu, or make it into a dropdown menu, and use [margin or padding utilities](/utilities/margin/) to give it the negative space you require.
...
| `container` | string, element, boolean | `false` | Appends the dropdown menu to a specific element when shown. Use `'body'` or `true` to append to the document body, which helps escape containers with `overflow: hidden`. The menu is moved back to its original position when hidden. Inside a modal dialog it goes into the dialog instead, because the rest of the page is inert. |
...
Dropdowns use local CSS variables on `.dropdown-menu` for enhanced real-time customization.

<ScssDocs file="scss/_dropdown.scss" capture="dropdown-tokens" />

The caret that indicates a dropdown's interactivity sizes itself from one token on `.dropdown-toggle`; the gap before it and its vertical alignment are computed from that width, so retuning it moves all three.

<ScssDocs file="scss/mixins/_caret.scss" capture="caret-tokens" />

Customization through CSS variables can be seen on the `.dropdown-menu-dark` class where we override specific values without adding duplicate CSS selectors.

<ScssDocs file="scss/_dropdown.scss" capture="dropdown-dark-tokens" />
...
<ScssDocs file="scss/_dropdown.scss" capture="dropdown-variables" />
