...
<Callout color="info">
**Built on Menu.** Since v6 the Dropdown plugin is a compatibility layer over the new [Menu](/components/menu/) component — same engine, keyboard handling and dismissal, with the v5 markup, classes and `*.coreui.dropdown` events preserved. New projects should prefer Menu; existing dropdown markup keeps working unchanged.
</Callout>
