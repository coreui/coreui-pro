...
<Example class="docs-example-offcanvas p-0 bg-light overflow-hidden" code={`<dialog class="offcanvas offcanvas-start" id="offcanvas" aria-labelledby="offcanvasLabel" open>
...
Change the appearance of offcanvas elements using utilities to better match various contexts, such as dark navbars. Here, we add `.text-bg-dark` to the `.offcanvas`; the close button takes the colour it inherits, so it needs nothing. If there are dropdowns inside, consider also adding `.dropdown-menu-dark` to `.dropdown-menu`.
...
<Example class="docs-example-offcanvas p-0 bg-body-secondary overflow-hidden" code={`<dialog class="offcanvas offcanvas-start text-bg-dark" id="offcanvasDark" aria-labelledby="offcanvasDarkLabel" open>
    <div class="offcanvas-header">
