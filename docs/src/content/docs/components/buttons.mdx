...
The loop that generates the [deprecated color classes](#deprecated-markup), pointing each one at the matching theme color. Set `$enable-deprecated-classes` to `false` to leave them out of the build.

<ScssDocs file="scss/buttons/_button.scss" capture="btn-deprecated-modifiers" />
