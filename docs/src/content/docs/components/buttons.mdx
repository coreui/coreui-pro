...
<ScssDocs file="scss/mixins/_button.scss" capture="btn-tokens" />

`.btn-link` rewrites the ones that make it read as a link:

<ScssDocs file="scss/mixins/_button.scss" capture="btn-link-tokens" />
...
<ScssDocs file="scss/mixins/_button.scss" capture="btn-variables" />

### Sass maps

`$button-sizes` lists the sizes that get a class. Each name generates `.btn-{size}` — plus the matching `.btn-group-{size}` and `.input-group-{size}` selectors — pointing the button's variables at the shared `--cui-btn-input-{size}-*` scale, so adding a name is enough once that scale defines it.

<ScssDocs file="scss/mixins/_button.scss" capture="btn-sizes" />
...
<ScssDocs file="scss/mixins/_button.scss" capture="btn-variants" />

### Sass mixins

`button-variant()` turns one entry of that map into the button's variables.

<ScssDocs file="scss/mixins/_button.scss" capture="btn-variant-mixin" />
...
The loop that generates the [deprecated color classes](#deprecated-markup), pointing each one at the matching theme color. It lives in the opt-in [`coreui-legacy.css`](/getting-started/install/#v5-compatibility-css), not in `coreui.css`.

<ScssDocs file="scss/_legacy.scss" capture="btn-deprecated-modifiers" />
