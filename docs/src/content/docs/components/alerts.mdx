...
- Add [`.transition-size`](/customize/transitions/#size-animations) and the alert animates its box, not only its opacity: it collapses as it is dismissed, so the content below rises into its place instead of jumping, and it grows from nothing when you insert it with `.fade` and `.show`. The class needs `interpolate-size`, so in a browser without it the alert fades in place as before, and it clips the alert's content for as long as it is on.

<Example code={`<div class="alert theme-info transition-size" role="alert">
    Dismiss this one and the paragraph below rises into its place.
    <button type="button" class="btn-close" data-coreui-dismiss="alert" aria-label="Close"></button>
  </div>
  <p class="mb-0">Content below the alert.</p>`} />
...
<ScssDocs file="scss/_alert.scss" capture="alert-tokens" />
...
The loop that turns every entry of the variants map into a `.alert-{variant}` class, re-pointing the alert's tokens at the roles that variant names.
...
<ScssDocs file="scss/_legacy.scss" capture="alert-modifiers" />
