...
The corners follow `--cui-progress-segment-border-radius`, `--cui-radius-2` by default — a full cap on the default width, and the same corner on a wider segment. It is capped at half the segment width, so a narrow segment stays a pill. Set it to `0px` for square segments, or anything in between. The value feeds the mask's `calc()`, so it has to be a length — a unitless `0` drops the whole mask.
...
<Example code={`<div class="progress progress-segmented mb-3" style="height: 1.5rem; --cui-progress-segment-border-radius: 0px">
  ...
  <div class="progress progress-segmented" style="height: 1.5rem; --cui-progress-segment-width: .75rem; --cui-progress-segment-border-radius: var(--cui-radius-5)">
