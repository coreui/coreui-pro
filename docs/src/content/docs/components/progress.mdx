...
Every `.progress-segmented` is initialized on page load — no toggle attribute needed. The bar's `aria-valuenow`, `aria-valuemin` and `aria-valuemax` are the value; leave `width` off the bar.
...
| Method | Description |
| --- | --- |
| `update` | Recomputes the filled segments from the bar's `aria-*` attributes. The plugin already watches those attributes, so this is for a change of `--cui-progress-segments` made from script. |
| `dispose` | Destroys the instance and removes the variable it set on the bar. |
