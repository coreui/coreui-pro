...
The loop that creates the [deprecated color classes](#deprecated-markup), pointing each one at the matching theme color. It lives in the opt-in [`coreui-legacy.css`](/getting-started/install/#v5-compatibility-css), not in `coreui.css`.

<ScssDocs file="scss/_legacy.scss" capture="callout-modifiers" />
