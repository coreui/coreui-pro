...
Use `.d-sidebar-narrow` to show content only in the narrow state and `.d-sidebar-narrow-none` to hide content in the narrow state. These utility classes replace the older brand-specific visibility helpers `.sidebar-brand-full`, `.sidebar-brand-narrow` and `.d-narrow-none`, which were removed in v6.
