...
<Example code={`<div class="card" style="width: 18rem;">
    <div class="card-body">
      <h5 class="card-title">Card title</h5>
      <h6 class="card-subtitle fg-2">Card subtitle</h6>
      <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
...
Use some of [available sizing utilities](/utilities/width/) to rapidly set a card's width.
...
You can instantly change the text arrangement of any card—in its whole or specific parts—with [text align classes](/utilities/text-alignment/).

`text-align` reaches the text but not the body's own flex items, so a button or a link stays at the start of the card. Move it with a [margin utility](/utilities/margin/): `.mx-auto` to centre it, `.ms-auto` to push it to the end.
...
<Example code={`<div class="card text-bg-inverse">
...
Add `.card-row` to lay a card out along the inline axis. The sections sit side by side with a single divider between each pair, so no wrapper markup is needed. For an image cap, use `.card-img-start` or `.card-img-end` to round the leading or trailing edge.
...
`.card-row` also moves the divider a header or footer draws onto the inline axis, so those work in a row too. It applies at every width; toggle the class yourself if the layout has to change at a breakpoint.
...
Use [border utilities](/utilities/border/) to change just the `border-color` of a card. Note that you can put `.text-{color}` classes on the parent `.card` or a subset of the card's contents as shown below.

<Example code={getData('theme-colors').map((c) => `<div class="card border-${c.name} mb-3" style="max-width: 18rem;">
    <div class="card-header">Header</div>
...
Use [border utilities](/utilities/border/) to change just the `border-top-color` of a card. Note that you can put `.text-{color}` classes on the parent `.card` or a subset of the card's contents as shown below.
...
Cards use local CSS variables on `.card` for enhanced real-time customization.
