...
`.badge-position-{block}-{inline}` pins a badge to a corner of its positioned parent, centred on that corner: `block` is `top` or `bottom`, `inline` is `start` or `end`, and the classes follow the writing direction.
