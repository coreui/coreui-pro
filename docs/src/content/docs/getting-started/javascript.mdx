...
The pickers build their field and calendar from other plugins. A default changed on a picker reaches its field and calendar, and wins over a default changed on the field or calendar plugin for the same option; a default changed only on the field or calendar plugin applies inside the pickers too.

```js
coreui.DatePicker.Default.selectionType = 'month' // the field and the calendar both pick months
coreui.DateRangeInput.Default.ariaStartLabel = 'Początek' // also names the start field of every DateRangePicker
```
...
$('#myTooltip').tooltip('setContent', { '.tooltip-inner': 'Another title' })

// a plugin is registered under the component's own name, so a two-word one
// is reached with brackets rather than a dot
$('#myPicker')['date-picker']('setDate', new Date(2027, 0, 15))
```
