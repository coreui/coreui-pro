...
It lists every class exactly one component stylesheet declares on its own.
Classes that only ever appear next to another class are absent — `.show` and
`.active` are states of the component they sit on, and `.theme-*` belongs to
`base.css`, which you load anyway.
