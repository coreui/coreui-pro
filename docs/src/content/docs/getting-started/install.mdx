...
`base.css` carries the design tokens on `:root` and [Reboot](/content/reboot/).
Every component stylesheet reads those tokens and Reboot sets the element
defaults components build on, so `base.css` always comes first — a component
file on its own has nothing to resolve its tokens against.
