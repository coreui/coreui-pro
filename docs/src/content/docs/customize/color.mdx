...
<table class="table table-swatches">
  ...
  <tbody>
    ...
    <tr>
      <td>
        **Border —** For component borders, dividers, and rules. Use `--cui-border-color-translucent`, a translucent mix of the body foreground, on surfaces that sit over other content.
      </td>
      <td class="ps-0">
...
Color reaches a component through two layers. The **theme colors** carry the hues and generate their own scales; the **neutral ramp** carries gray, which every surface, divider and muted text tone reads. There is no third palette underneath them. The pipeline, end to end:

1. **Theme colors** (`$primary`, `$success`, … in `scss/_theme.scss`) each get a 025–975 scale of `color-mix()` tokens on `:root`, mixed in the browser from the color's own token — `--cui-primary-300`.
2. **`$theme-colors`** maps each semantic name to a family of slots (`base`, `contrast`, `fg`, `fg-emphasis`, `bg-subtle`, `bg-muted`, `border`, `focus-ring`), mostly reading that scale through `light-dark()` so every slot adapts to the color mode.
3. The slots are emitted as **`--cui-{name}-{slot}`** tokens — `--cui-primary-bg-subtle` — the ones the table at the top of this page shows.
4. **`.theme-{name}`** classes remap them onto generic **`--cui-theme-{slot}`** tokens that components read, so one class restyles any theme-aware component.
5. **`$gray`** (`scss/_colors.scss`) gets the same scale through the same generator, and feeds the `bg`, `fg` and `border` ladders plus the `secondary` and `inverse` theme colors.

Override `--cui-primary-base` and the whole family follows, in both color schemes. The theme layer is documented in full on [Theme](/customize/theme/), and the mechanics of the slot indirection on [Architecture](/customize/architecture/#theme-slots).

## All colors

v6 ships one hue-free scale, the neutral ramp, plus black and white. Every other color the library draws comes from a theme color, so there is no separate palette of `blue`, `teal` or `pink` variables to pick from. If you need a hue the theme does not cover, add it as a theme color — see [adding a theme color](/customize/theme/#adding-a-theme-color) — and it gets the same scale and the same slots.

Be sure to monitor contrast ratios as you customize colors. Each swatch below carries three: against its own text color, against white, and against black.
...
<div class="row font-monospace">
  ...
  <div class="col-md-4 mb-3">
    <div class="p-3 mb-2 docs-black fg-white">
...
The tints and shades are not Sass variables any more. Every theme color and the
neutral ramp get a 025–975 scale of `color-mix()` custom properties on `:root`,
mixed in the browser from the color's own token — override `--cui-primary-base`
or `--cui-gray` and the whole family follows, in both color schemes. See
[the theme colors page](/customize/theme/).

The gray stops stay available as Sass values as well, because a few places
cannot read a token: channel math, colors baked into an SVG, and the docs
engine.
...
CoreUI for Bootstrap's source Sass files include three maps to help you quickly and easily loop over a list of colors and their values.

- `$colors` lists the hue-free base colors — in v6 that is `gray` alone
- `$theme-colors` lists all semantically named theme colors (shown above)
- `$grays` lists all tints and shades of gray

Within `scss/_colors.scss`, you'll find the color variable and the Sass map:

<ScssDocs file="scss/_colors.scss" capture="color-variables" />

<ScssDocs file="scss/_colors.scss" capture="colors-map" />

Adding an entry here gives it the same runtime scale every theme color gets, without any of the semantic slots. Reach for `$theme-colors` instead when the color needs a foreground, a border and a subtle background that travel together.
...
The numbered shades of the hues — `$yellow-300`, `$indigo-900` and the rest — are
gone in v6, along with the hues themselves. Each theme color gets its 025–975
scale as `color-mix()` tokens on `:root` instead, so read
`var(--cui-primary-300)` where you used to write `$blue-300`. See
[the theme colors page](/customize/theme/).

[Color](/utilities/colors/) and [background](/utilities/background/) utility classes are also available for setting `color` and `background-color` from the theme colors.
