...
JavaScript adds and removes a small, fixed set of classes; `.fade` and `.size` are the two you put on the markup yourself. Style these and your customizations follow the component.
...
| `.size` | The box itself rather than the opacity: an element with it collapses as it leaves and grows as it arrives. Opt-in, on your own markup, and only where the browser can interpolate a size — see [size animations](#size-animations). |
...
<ScssDocs file="scss/_transitions.scss" capture="size-tokens" />

A hidden `.size` element ends up `display: none`, the way a closed collapse does, so it leaves the layout and the accessibility tree instead of staying as a clipped, focusable strip. `display` rides the transition with `allow-discrete`, which is what keeps the element on screen until the box has reached zero.
...
The hidden state is emitted in the `utilities` layer, because it has to zero the spacing a utility may have set: `<div class="alert fade size show py-5 mb-5">` collapses its padding and its margin like any other alert.

The class carries a transition of its own, which replaces the one a component declares unless that component's rule is the more specific of the two. The alert's is, so the alert instead names the sizes in `--cui-alert-transition-property` and keeps its own duration and easing, and `<div class="alert fade size show">` is the whole opt-in. Do the same on any other component whose timing you want to keep.

An element that sits in the page's initial HTML with `.fade`, `.show` and `.size` plays the entrance on first paint, the way `@starting-style` already fades an alert in. Insert it without `.show` and add the class afterwards if you would rather it did not.
