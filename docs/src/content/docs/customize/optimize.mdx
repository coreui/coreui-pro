...
Two blocks of that stack are also available without Sass: `coreui-utilities`
writes the utilities layer from `utilities.json`, trimmed to the classes your
...
The v5 spellings this stack does not carry are their own entrypoint — add
`@use "coreui-legacy";` after it while your markup still needs them, and drop
that line once it does not. See [v5 compatibility CSS](/getting-started/install/#v5-compatibility-css).
...
export default {
  ...
  safelist: {
    ...
    greedy: [/^combobox/, /^list-box/, /^date-/, /^time-/, /^password-/, /^rating/, /^dialog/, /^modal/, /^carousel/, /^stepper/, /^form-/, /^is-/]
  }
}
```
