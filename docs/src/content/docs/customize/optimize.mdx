...
Utilities are not in that stack: `coreui.css` and `coreui-utilities.css` take
their utilities layer from `utilities.json` at build time, not from Sass. A Sass
build of your own emits it by loading the API module after the library
(`@use "@coreui/coreui/scss/utilities/api";`), and `coreui-utilities` writes the
same stylesheet — trimmed to the classes your markup uses — without Sass at all.
See [the utility API](/utilities/api/#without-sass).

`root` carries no rules in that stack either. The palette, the root tokens, the
breakpoint and spacer tokens and the `.theme-*` classes are written from
`tokens.json` at build time and composed into the compiled stylesheet where
Sass used to emit them; `@forward "root"` is there for the maps the components
read. A Sass build of your own emits the four layers by loading
`@use "@coreui/coreui/scss/tokens/api";` after the library. See
...
CoreUI for Bootstrap's JavaScript includes every component in our primary dist files (`coreui.js` and `coreui.min.js`), and even our primary dependency (Floating UI) with our bundle files (`coreui.bundle.js` and `coreui.bundle.min.js`). While you're customizing via Sass, be sure to remove related JavaScript.
...
This way, you're not including any JavaScript you don't intend to use for components like buttons, carousels, and tooltips. If you're importing dropdowns, tooltips or popovers, be sure to list the `@floating-ui/dom` dependency in your `package.json` file.
