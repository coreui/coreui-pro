...
The build writes the tokens and the `.theme-*` classes behind this map from `tokens.json`, so the map above documents their shape rather than compiling into the shipped stylesheet — see [CSS variables](/customize/css-variables/#the-root-layer-comes-from-tokensjson) for what that changes about overriding them.
...
```css
:root {
  --cui-primary-base: #6f42c1; /* the scale, the slots and every component follow */
}
```
