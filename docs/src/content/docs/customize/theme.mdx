...
`npm run tokens` writes this map from `tokens.json`, so a color added there arrives in the map, in the tokens and in the `.theme-*` classes at once — see [CSS variables](/customize/css-variables/#the-root-layer-comes-from-tokensjson).
