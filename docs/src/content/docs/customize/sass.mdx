...
The library's configuration is split across four modules, each importable on its
...
scale; and `scss/root` emits every token on `:root`. The pre-v6 `scss/variables`
and `scss/maps` paths still resolve to the same modules.
