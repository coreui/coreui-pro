...
Here are the variables we include (note that the `:root, :host` selector is required) that can be accessed anywhere CoreUI for Bootstrap's CSS is loaded. The build writes this layer from [`tokens.json`](#the-root-layer-comes-from-tokensjson), not from a Sass loop, and appends it to the compiled dist files. Most of them pick their light and dark value in a single declaration through the CSS [`light-dark()`](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value/light-dark) function, so one declaration covers both [color modes](/customize/color-modes/).
...
<ScssDocs file="scss/tokens/_api.scss" capture="root-dark-mode-vars" />

### The root layer comes from `tokens.json`

<AddedIn version="6.0.0" /> Every declaration above is written by the build from `tokens.json` — the palette and its scales, the theme color slots, the type, radius, border, z-index and shadow scales, and the shared `--cui-btn-input-*` and `--cui-control-*` tokens. The Sass maps behind them stay in `scss/_root.scss`, `scss/_colors.scss` and `scss/_theme.scss` as the documented shape of each group, and `scss/tokens/_api.scss` is the Sass that emits them; the build refuses a byte of drift between the two.

Two consequences for a Sass build of your own:

- `@use "@coreui/coreui/scss/coreui" with ($primary: …, $body-bg: …, $font-family-sans-serif: …)` no longer reaches `:root`. Set the token in your own CSS instead — every one of them is a plain custom property on `:root`, so `--cui-primary-base: hotpink` is the whole change and it needs no compiler.
- If you would rather keep the Sass path, load the token API module after the library. It emits the same four layers from the Sass maps, so `with ()` overrides apply again:
...
The generated layers — the palette, the root tokens, the theme classes and the utilities — take the prefix from the build rather than from that variable, so respelling it in Sass alone leaves them on `cui-` and the two halves stop matching. A theme that renames the prefix declares it once for both halves; `scss/themes/bootstrap/bootstrap.scss` and its `tokens.json` are the worked example.
