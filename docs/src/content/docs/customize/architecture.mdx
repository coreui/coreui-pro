...
Each layer reads the one below it, so a change low down travels upward: retint `--cui-primary-500` and every slot of that theme color follows, along with every component reading those slots.
...
- **Utilities win over components** because `utilities` comes last — no `!important` involved, and no switch to bring it back.
...
- **`colors` is where the palette ships**, below everything else, so your own `@layer colors` block sits in the same layer and overrides a token like `--cui-primary-500` by source order alone.
...
The global decisions — the palette, the color scale stops, the eight slots of every theme color, the background, foreground and border families, and the spacing, radius, breakpoint, container, border-width, font-weight, type, z-index and aspect-ratio scales — live in `tokens.json` at the package root, in the [W3C Design Tokens](https://tr.designtokens.org/format/) format. The Sass maps in `scss/_colors.scss`, `scss/_theme.scss` and `scss/_config.scss` are generated from it (`npm run tokens`) and CI fails when they drift, so the JSON is the file to read when you need the values outside Sass: a Figma sync, a token bridge to another framework, or a typed list of what `:root` declares. The Sass maps keep their `!default` overrides, so `@use … with (…)` and `map.merge()` customisation work exactly as before.

Four value shapes appear in the file. A plain string is a value or a `{path}` alias, which the generator writes as `var(--cui-…)` so the reference resolves in the browser (or as the Sass variable, in the few maps that are still multiplied at build time); `{ "light": …, "dark": … }` becomes `light-dark()`; `{ "color": …, "alpha": … }` becomes the `color-mix()` behind the focus ring; `{ "multiply": [alias, factor] }` becomes `calc(var(--cui-…) * factor)`. The CSS syntax belongs to the generator, not to the JSON.

The utility classes are described the same way: `utilities.json` lists every utility — its property, class, values and the responsive, print, state and child-selector switches — and `build/utilities.mjs` writes the utilities stylesheet from it and from the token scales. During the v6 alpha the shipped stylesheet is still compiled by Sass from the `$utilities` map; the generator is checked against that output in CI so the two cannot drift.
...
The slot values come from per-color scales generated with `color-mix()` from each color's base token, and the interactive states are derived at run time with relative color syntax — `oklch(from var(--cui-primary-base) calc(l * .85) c h)` — so overriding `--cui-primary` alone re-colors the subtle backgrounds, borders, emphasis text, hover and active states with it. The color system is documented on [Color](/customize/color/), the scheme switching on [Color modes](/customize/color-modes/).
