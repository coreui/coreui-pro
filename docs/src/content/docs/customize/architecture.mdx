...
Each layer reads the one below it, so a change low down travels upward: retint `--cui-primary-base` and every slot of that theme color follows, along with every component reading those slots.
...
- **`colors` is where the color scales ship**, below everything else, so your own `@layer colors` block sits in the same layer and overrides a token like `--cui-primary-300` by source order alone.
