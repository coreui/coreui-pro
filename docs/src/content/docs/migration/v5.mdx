...
Learn more by reading the new [color modes documentation](/customize/color-modes/).
...
- Adds new `_variables-dark.scss` stylesheet to house dark-mode specific overrides. This stylesheet should be imported immediately after the existing `_config.scss` file in your import stack.
...
- <span class="badge text-warning-emphasis bg-warning-subtle">Deprecated</span> The `.alert-variant()` mixin is now deprecated. We now [use a Sass loop](/components/alerts/#sass-loops) directly to modify the component's default CSS variables for each variant.
...
- <span class="badge bg-danger">Breaking</span> Removed option `format`, use `inputDateFormat` instead of. [See the docs for an example.](/forms/date-picker/#custom-formats)
...
- <span class="badge bg-danger">Breaking</span> Removed option `format`, use `inputDateFormat` instead of. [See the docs for an example.](/forms/date-range-picker/#custom-formats)
...
- <span class="badge text-warning-emphasis bg-warning-subtle">Deprecated</span> The `.dropdown-menu-dark` class has been deprecated and replaced with `data-coreui-theme="dark"` on the dropdown or any parent element. [See the docs for an example.](/components/dropdowns/#dark-dropdowns)

#### List group

- List group item variants are now styled via CSS variables.

- <span class="badge text-warning-emphasis bg-warning-subtle">Deprecated</span> The `.list-group-variant()` mixin is now deprecated. We now [use a Sass loop](/components/list-group/#sass-loops) directly to modify the component's default CSS variables for each variant.

#### Navbar

- <span class="badge text-warning-emphasis bg-warning-subtle">Deprecated</span> The `.navbar-dark` class has been deprecated and replaced with `data-coreui-theme="dark"` on the navbar or any parent element. [See the docs for updated examples.](/components/navbar/#color-schemes)
...
We've also introduced a new `.progress-stacked` class to more logically wrap [multiple progress bars](/components/progress/#multiple-bars) into a single stacked progress bar.
...
- Added new [`.z-*` utilities](/utilities/z-index/) for `z-index`.

- [Box shadow utilities](/utilities/shadows/) (and Sass variables) have been updated for dark mode. They now use `--cui-body-color-rgb` to generate the `rgba()` color values, allowing them to easily adapt to color modes based on the specified foreground color.
...
- Added new `.d-inline-grid` [display utility](/utilities/display/).
