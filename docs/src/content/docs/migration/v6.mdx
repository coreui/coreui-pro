...
- <span class="badge text-danger-emphasis bg-danger-subtle">Breaking</span>
  ...
  **every** colour now carries the full thirteen stops, and their percentages
  moved to the curve the gray ramp used: the light stops are much lighter than
  before (measured: `-400` moves 80 per RGB channel, `-300` 57, `-200` 38,
  `-100` 19; the dark side moves 6 to 42). The grays themselves are unchanged
  bar a rounding step in the light band and up to 12 in the dark, where the
  hand-picked ramp was more chromatic than one base can be.
