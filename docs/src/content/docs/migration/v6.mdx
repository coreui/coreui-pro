...
- <span class="badge text-danger-emphasis bg-danger-subtle">Breaking</span>
  **`.sidebar-sm`, `.sidebar-lg` and `.sidebar-xl` are gone, with the
  `$sidebar-widths` map.** The width is one token: set `--cui-sidebar-width`
  on the sidebar (`style="--cui-sidebar-width: 20rem"` replaces `.sidebar-lg`);
  the space reserved next to it follows the same token.
- <span class="badge text-danger-emphasis bg-danger-subtle">Breaking</span>
  **`.sidebar-nav-tree`, `.sidebar-body` and `.sidebar-form` are removed.** The
  tree variant was decoration nothing shipped used; the other two carried no
...
- **Every palette colour gets a 100–900 scale of tokens in `:root`**, mixed at
  ...
  `--cui-primary-900`, for each key of `$colors`. Knobs: `$color-tints`,
  `$color-shades`, `$color-mix-space` (`oklch`), `$tint-color`, `$shade-color`.
  Override `$primary` — or the `--cui-primary` token at runtime — and the whole
  family retints. `secondary` and `inverse` are built from the grays and carry
  no scale.
- <span class="badge text-danger-emphasis bg-danger-subtle">Breaking</span>
