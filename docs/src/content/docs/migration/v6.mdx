...
#### The deprecated colour classes are behind a flag

- **`$enable-deprecated-classes` drops the v5 colour modifiers from the build.**
  It defaults to `true`, so nothing changes unless you set it: with the flag on,
  the compiled CSS is byte-identical to a build without it. Set it to `false`
  once your markup uses the theme classes and `.alert-{color}`,
  `.btn-{color}`, `.btn-outline-{color}`, `.btn-ghost-{color}`,
  `.callout-{color}`, `.chip-{color}` and `.list-group-item-{color}` leave the
  stylesheet. They are removed outright in v7.
- **The buttons lose the colour spelling in two places, not one.** Besides the
  loop that points each class at a theme colour, `btn-variant-selectors()`
  appends the colour classes to the selector list that carries a variant's base
  declarations — the flag gates both, so `.btn-primary` disappears whole rather
  than surviving as a rule with no colour.
...
#### The utilities stylesheet is written from `utilities.json`

<span class="badge text-danger-emphasis bg-danger-subtle">Breaking</span>

- **`scss/coreui` and `scss/coreui-utilities` no longer load the utilities API
  module.** The utilities layer of `dist/css/coreui.css` and
  `dist/css/coreui-utilities.css` is generated from `utilities.json` and the
  token scales in `tokens.json`, and appended after Sass has written the rest —
  byte for byte what the Sass loop used to emit. If you compile the library
  yourself, load the API module after it, or the classes are missing entirely:
...
- **A `$utilities` override no longer reaches the shipped stylesheet.**
  `@use "@coreui/coreui/scss/coreui" with ($utilities: …)` and the
  `map.merge($utilities, …)` partial still work in *your* Sass build — the map,
  the `generate-utilities-loop()` mixin and `scss/utilities/_api.scss` all
  remain — but they change nothing in `coreui.css` or `coreui-utilities.css`.
  The same change as data goes in a JSON file merged over `utilities.json` with
  the same rules (a key replaces the whole group, `null` removes it, a new key
  is appended):
...
- **Three Sass settings stop reaching the utilities.** The generator reads
  `utilities.json` and `tokens.json`, so `@use "config" with ()` overrides of
  `$prefix`, `$breakpoints` and `$enable-reduced-motion` no longer change the
  generated layer: the class prefix comes from the build rather than from the
  Sass variable, the breakpoints come from the `breakpoint` tokens, and the
  reduced-motion rule that stands the animation utilities down is always
  written. Move the breakpoints to `tokens.json`; a build that has to drop the
  reduced-motion rule emits the layer itself through the API module, as above.

#### The root layer and the theme classes are written from `tokens.json`

<span class="badge text-danger-emphasis bg-danger-subtle">Breaking</span>

- **Four layers of the compiled stylesheet no longer come from Sass.** The
  palette and its scales (`@layer colors`), the root tokens and the dark-mode
  block (`@layer root`), the breakpoint and spacer tokens, and the
  `.theme-*` / `.theme-reset` classes (`@layer helpers`) are written from
  `tokens.json` and composed into `dist/css/coreui.css`,
  `coreui-reboot.css`, `coreui-utilities.css`, `coreui-grid.css` and
  `themes/bootstrap/bootstrap.css` at the position Sass used to emit them —
  byte for byte what the Sass loops used to write.

- **A `with ()` override no longer reaches `:root`.**
  `@use "@coreui/coreui/scss/coreui" with ($primary: …, $body-bg: …,
  $font-family-sans-serif: …, $radius: …, $spacer: …, $zindex-levels: …)` and
  the theme maps (`$theme-colors`, `$theme-bgs`, `$theme-fgs`,
  `$theme-borders`, `$color-tokens`, `$root-tokens`) still work in *your* Sass
  build — every map, function and mixin remains, and the docs still quote them
  — but they change nothing in the shipped stylesheets. Two ways to make the
  change instead:

  ```css
  /* the token is a plain custom property, so this needs no compiler */
  :root {
    --cui-primary-base: hotpink;
    --cui-spacer: 1.25rem;
  }
  ```

  ```diff
    /* or keep Sass: the API module emits the four layers from the maps */
    @use "@coreui/coreui/scss/coreui" with ($primary: hotpink);
  + @use "@coreui/coreui/scss/tokens/api";
  ```

- **A theme states its palette as JSON.** `scss/themes/bootstrap/tokens.json`
  is the worked example: it overlays `tokens.json` with Bootstrap's
  hand-picked gray ramp and theme colors, and the build passes the theme's
  `bs-` prefix and `-bs-` data infix to the same generator, so
  `bootstrap.css` no longer loads the Sass utilities API module either.
...
- <span class="badge text-danger-emphasis bg-danger-subtle">Breaking</span>
  **The colour utilities take upstream's names, and opacity is a step of the
  ...
  `.text-reset`, `.text-muted`, `.text-body-secondary`, `.text-body-tertiary`,
  `.text-black-50`, `.text-white-50`, `.bg-{color}-subtle`, `.bg-{color}-muted`,
