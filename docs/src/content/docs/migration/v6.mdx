...
- <span class="badge text-danger-emphasis bg-danger-subtle">Breaking</span>
  **The palette is written in `oklch()` and mixes in `oklch`.** The base colours
  ...
  it (measured: 5.9 per RGB channel on average, 28 at most). `$tint-color` and
  `$shade-color` drop the hue of the colour they mix towards, so a shade darkens
  without rotating towards our near-black's blue cast.
