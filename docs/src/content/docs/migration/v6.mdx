...
- **Three Sass settings stop reaching the utilities.** The generator reads
  ...
  generated layer: the class prefix is fixed at `cui-`, the breakpoints come
  from the `breakpoint` tokens, and the reduced-motion rule that stands the
  animation utilities down is always written. Move the breakpoints to
  `tokens.json`; a build that has to respell the prefix or drop the
  reduced-motion rule emits the layer itself through the API module, as above.
  The Bootstrap theme (`scss/themes/bootstrap/bootstrap.scss`) does exactly
  that, because it writes `bs-`.
