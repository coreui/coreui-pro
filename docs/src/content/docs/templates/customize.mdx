...
You will find CoreUI's variables in `node_modules/@coreui/coreui/scss/` (or `node_modules/@coreui/coreui-pro/scss/` if you use CoreUI PRO): the global settings in `_config.scss`, the palette in `_colors.scss`, and the theme colours in `_theme.scss`. Some variables are set to `null`, these variables don't output the property unless they are overridden in your configuration. You can also find a specific component variables list in **Customizing** section ex. [Alerts - Customizing](/components/alerts/#customizing)
