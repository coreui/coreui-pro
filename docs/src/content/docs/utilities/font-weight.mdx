...
Each entry is one weight: the key names the `.fw-*` class and the matching `--cui-font-weight-*` token, and `null` drops both.

<ScssDocs file="scss/_config.scss" capture="font-weights" />
