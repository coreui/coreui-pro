...
| `ariaLabels` | array, null | `null` | Accessible name of each handle, in order. Handles are otherwise indistinguishable to a screen reader, so a slider with more than one wants this. A shorter array leaves the remaining handles on their generated names, such as `Maximum value`. |
...
<ScssDocs file="scss/_range-slider.scss" capture="range-slider-vertical-css-vars" />
...
<ScssDocs file="scss/_range-slider.scss" capture="range-slider-variables" />
