...
<Callout color="info">
**Note:** The `<input class="chip-input-field" />` is created automatically by the component when initialized. You can also add it manually in the markup if preferred. Additionally, a hidden input is generated automatically to carry the chip values. It is submitted with the form only when you give it a name through `data-coreui-name` (e.g., `data-coreui-name="chipInputBasicExample"`); without one the component stays out of the payload.
</Callout>
