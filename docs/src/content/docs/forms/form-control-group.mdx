...
the frame up without any extra wiring. A sibling `.invalid-feedback` follows the
`.is-invalid` class wherever it sits, on the group or on the control inside it,
and a `.valid-feedback` waits until nothing is invalid, the frame included. The validation icon is suppressed inside
...
<Example code={`<div class="form-control-group is-invalid">
  ...
  <div class="invalid-feedback">Please pick a different value.</div>`} />
