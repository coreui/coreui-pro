...
| `data-coreui-picker-toggle` | a `button` or any element | Opens and closes the calendar. Gets `aria-expanded`, `aria-controls` and `aria-haspopup`, and `ariaPickerLabel` as its name unless it has one. |
