...
import multiSelectArrayData from './snippets/multi-select-array-data.js?raw'
import multiSelectCustomOptions from './snippets/multi-select-custom-options.js?raw'
import multiSelectHeader from './snippets/multi-select-header.js?raw'
import multiSelectIndeterminate from './snippets/multi-select-indeterminate.js?raw'
import multiSelectSelectionLimit from './snippets/multi-select-selection-limit.js?raw'
...
<Example pro code={`<select id="ms1" data-coreui-multi-select multiple data-coreui-search="true">
...
<Example pro code={`<select data-coreui-multi-select multiple data-coreui-search="true">
    <option value="0">Angular</option>
...
<Example pro code={`<div class="form-field">
    ...
    <select id="multiSelectField" name="stack" data-coreui-multi-select multiple>
      <option value="0">Angular</option>
      <option value="1">Bootstrap</option>
      <option value="2">React.js</option>
      <option value="3">Vue.js</option>
...
<Example pro code={`<select id="multiSelect" name="multiSelect"></select>`} sandboxJs={multiSelectArrayData} />

We use the following JavaScript to set up our multi-select:

<Code code={multiSelectArrayData} lang="js" />
...
<Example pro code={`<select data-coreui-multi-select multiple>
    <option value="0">Angular</option>
    <option value="1">Bootstrap</option>
    <option value="2">React.js</option>
    <option value="3">Vue.js</option>
...
Typing reaches the search box from anywhere in the component — the control, the options panel, the select all button — so it never depends on where the focus happens to be. That is how `data-coreui-search="true"` behaves on its own.
...
Add `data-coreui-global-search="false"` to keep typing local. The search box then filters only while it has the focus, and a keystroke over the options list jumps to the first option starting with it.

<Example pro code={`<select data-coreui-multi-select multiple data-coreui-search="true" data-coreui-global-search="false">
    <option value="0">Angular</option>
    <option value="1">Bootstrap</option>
    <option value="2">React.js</option>
    <option value="3">Vue.js</option>
...
<Example pro code={`<select id="multiple-select-text" data-coreui-multi-select multiple data-coreui-selection-type="text" data-coreui-search="true">
    <option value="0">Angular</option>
    <option value="1">Bootstrap</option>
    <option value="2">React.js</option>
    <option value="3">Vue.js</option>
...
<Example pro code={`<select id="multiple-select-tag" data-coreui-multi-select multiple data-coreui-selection-type="tags" data-coreui-search="true">
    <option value="0">Angular</option>
    <option value="1">Bootstrap</option>
    <option value="2">React.js</option>
    <option value="3">Vue.js</option>
...
<Example pro code={`<select id="multiple-select-counter" data-coreui-multi-select multiple data-coreui-selection-type="counter" data-coreui-search="true">
    <option value="0">Angular</option>
    <option value="1">Bootstrap</option>
    <option value="2">React.js</option>
    <option value="3">Vue.js</option>
...
With the default `indicator: 'checkbox'` the button shows the same tri-state indicator as the options and mirrors the overall selection — `none` when nothing is selected, `all` when everything is, and `indeterminate` in between; `aria-pressed` carries the same three states. With `indicator: 'none'` the button is a plain text toggle. It sits in the panel header, outside the options list, so <kbd>Tab</kbd> reaches it — the arrows stay on the options.

<Example pro code={`<select id="multiSelectSelectAll" data-coreui-multi-select multiple data-coreui-search="true">
    <option value="0">Angular</option>
    <option value="1">Bootstrap</option>
    <option value="2">React.js</option>
    <option value="3">Vue.js</option>
...
<Example pro code={`<select id="multiSelectSelectAllMode" data-coreui-multi-select multiple data-coreui-search="true" data-coreui-select-all-mode="filtered">
    <option value="0">Angular</option>
    <option value="1">Bootstrap</option>
    <option value="2">React.js</option>
    <option value="3">Vue.js</option>
...
<Example pro code={`<select id="multiSelectHideSelectAll" data-coreui-multi-select multiple data-coreui-search="true">
  <option value="0">Angular</option>
  <option value="1">Bootstrap</option>
  <option value="2">React.js</option>
  <option value="3">Vue.js</option>
...
<Example pro code={`<select id="multiSelectKeepSelectAll" data-coreui-multi-select multiple data-coreui-search="true" data-coreui-hide-select-all-on-search-no-results="false">
  <option value="0">Angular</option>
  <option value="1">Bootstrap</option>
  <option value="2">React.js</option>
  <option value="3">Vue.js</option>
...
<Example pro code={`<select id="multiSelectSelectionLimit" data-coreui-multi-select multiple data-coreui-search="true" data-coreui-selection-limit="3">
    <option value="0">Angular</option>
    <option value="1">Bootstrap</option>
    <option value="2">React.js</option>
    <option value="3">Vue.js</option>
  ...
  </select>`} sandboxJs={multiSelectSelectionLimit} />
...
<Code code={multiSelectSelectionLimit} lang="js" />
...
<Example pro code={`<select id="single-select-example" data-coreui-multi-select data-coreui-multiple="false">
    <option value="0">Angular</option>
    <option value="1">Bootstrap</option>
    <option value="2">React.js</option>
    <option value="3">Vue.js</option>
...
<Example pro code={`<select id="ms1-disabled" data-coreui-multi-select multiple data-coreui-search="true"  data-coreui-disabled="true">
    <option value="0">Angular</option>
    <option value="1">Bootstrap</option>
    <option value="2">React.js</option>
    <option value="3">Vue.js</option>
...
<Example pro code={`<div class="row">
  ...
  </div>`} sandboxJs={multiSelectCustomOptions} />

We use the following JavaScript to set up our multi-select:

<Code code={multiSelectCustomOptions} lang="js" />

## Custom dropdown header

<AddedIn version="5.26.0" />

Use the `headerTemplate` option to fully customize the dropdown header — the area above the options list — including rendering several action buttons. It replaces the built-in select all button and renders independently of `selectAll`. The template function receives a `state` object (`{ selected, total, filtered, filteredSelected }`) and an `actions` object with bound component methods (`selectAll`, `selectFiltered`, `deselectFiltered`, `deselectAll`). Return an `HTMLElement` and wire your own listeners to the `actions`, or return an HTML string for simple custom content. A string goes through the sanitizer, which drops interactive elements such as `<button>`, and nothing wires the `actions` to it, so controls belong on the `HTMLElement` path. The header re-renders on every selection change and search filter, so labels and `disabled` states stay up to date.

<Example pro code={`<select id="myMultiSelectHeader" data-coreui-multi-select></select>`} sandboxJs={multiSelectHeader} />

We use the following JavaScript to set up our multi-select:

<Code code={multiSelectHeader} lang="js" />

## Selectable groups

<AddedIn version="5.26.0" />

Enable `optionsGroupsSelectable` to turn each group label into its own tri-state button (`aria-pressed` `false` / `mixed` / `true`) that toggles the group's visible options. Because the indicator is rendered with CSS, it supports a third, `indeterminate` state — shown when only some of a group's options are selected — without any real `<input />` element. The same tri-state logic drives the header [select all](#select-all) button. Group labels join the arrow-key order, and the indicator on options, groups and select all follows the one `indicator` option; the whole feature requires `multiple`.

<Example pro code={`<select id="myMultiSelectIndeterminate" data-coreui-multi-select></select>`} sandboxJs={multiSelectIndeterminate} />
...
```html
<select id="ms1" name="stack" multiple data-coreui-multi-select data-coreui-search="true">
  <option value="0">Angular</option>
  <option value="1">Bootstrap</option>
  <option value="2">React.js</option>
  <option value="3">Vue.js</option>
...
| `ariaPickerLabel` | string | `'Toggle options list'` | Accessible label of the picker button. |
...
| `container` | string, element, false | `false` | Appends the dropdown to a specific element. Example: `container: 'body'`. Inside an open dialog it goes into the dialog instead, because the rest of the page is inert while a modal dialog is open. |
...
| `globalSearch` | boolean | `true` | Routes typing to the search box from anywhere in the component, so a keystroke never depends on where the focus sits. Set to `false` to keep it local: the search box then filters only while it has the focus, and typing over the options list jumps to the first option that starts with what was typed. Needs `search`. |
| `headerTemplate` | function, null | `null` | Custom template function for rendering the dropdown header (the area above the options list). Replaces the built-in select all button and renders independently of `selectAll`. Receives a `state` object (`{ selected, total, filtered, filteredSelected }`) and an `actions` object (`{ selectAll, deselectAll, selectFiltered, deselectFiltered }`) with bound component methods. Return an `HTMLElement` (wire your own listeners using `actions`, supports multiple controls) or an HTML string (sanitized, no actions wired). Re-renders on every selection change and search filter. |
...
| `indicator` | string | `'checkbox'` | Sets the marker used by the options, the selectable group labels and the select all button: `'checkbox'` or `'none'`. |
| `invalid` | boolean | `false` | Toggle the invalid state for the component. |
| `multiple` | boolean | `true` | It specifies that multiple options can be selected at once. |
| `name` | string, null | `null` | Name of the native select the selection is submitted under. A `name` written on the `<select>` itself is honoured too, and this option wins over it. With neither the selection is not submitted with the form. |
| `options` | boolean, array | `false` | List of option elements. |
| `optionsGroupsSelectable` | boolean | `false` | When enabled (with `multiple`), each option group label becomes a tri-state button: clicking it toggles the group's visible options, and it reflects `none` / `all` / `indeterminate` based on the group's selected options. |
...
| `pickerIcon` | string, boolean | `true` | The button that toggles the options list: `true` for the built-in icon, an inline SVG string for your own (sanitized), `false` for no button. Clicking the control only opens the list, so without the button a pointer closes it by clicking away. |
...
| `search` | boolean | `false` | Renders a search box in the control and filters the options as the user types. It takes the focus when the panel opens, and typing anywhere in the component reaches it unless `globalSearch` is off. |
...
| `selectedLabel` | string, function | `` (count) => `${count} item(s) selected` `` | The counter selection label. A function `(count, total) => string`, or a string in which `{count}` and `{total}` are replaced. |
...
| `setConfig` | Merges the given options over the current configuration and rebuilds the interface. Options you leave out keep their current value. |
...
| Event | Description |
| --- | --- |
| `change.coreui.multi-select` | Fires immediately when an option is selected or deselected. |
...
myMutliSelect.addEventListener('change.coreui.multi-select', event => {
...
The options panel is a dialog named by the control. Opening it moves the focus inside — onto the search box when `search` is on, otherwise onto the option the list points at, which is the selection the user left or the first option. The options are focusable one at a time: the arrows move the focus itself rather than a highlight read through `aria-activedescendant`. <kbd>Tab</kbd> cycles through what the panel offers and the search box in the control, and reaches nothing outside them until the panel closes, which <kbd>Escape</kbd> does, returning the focus to the control.

### Keyboard shortcuts

| Key | Action |
| --- | --- |
| <kbd>Tab</kbd> | Move between the search box, the select all button and the options list; the focus stays in the open panel |
| <kbd>Enter</kbd>/<kbd>Space</kbd> | Open the panel from the control, or select the focused option; <kbd>Enter</kbd> in the search box takes the option the list points at |
| <kbd>Arrow Up</kbd>/<kbd>Arrow Down</kbd> | Move through the options (and the selectable group labels), or enter the list from the search box |
| <kbd>Shift</kbd> + <kbd>Arrow Up</kbd>/<kbd>Arrow Down</kbd> | Extend the selection while moving (multiple selection) |
| <kbd>Home</kbd>/<kbd>End</kbd> | Focus the first/last option (not while typing in the search box) |
| <kbd>Ctrl</kbd>/<kbd>Cmd</kbd> + <kbd>A</kbd> | Select every option the list shows, from the options list (multiple selection) |
| <kbd>Escape</kbd> | Close the panel and return the focus to the control |
| <kbd>Backspace</kbd>/<kbd>Delete</kbd> | Remove the last selected item (when the search box is empty) |
...
<ScssDocs file="scss/forms/_form-multi-select.scss" capture="form-multi-select-tokens" />

The options panel is the shared combobox surface, which only sizes itself:

<ScssDocs file="scss/_combobox.scss" capture="combobox-tokens" />

Everything inside it — options, section labels, checkbox indicators and the select all button — is a [List box](/components/list-box/) and takes its `--cui-list-box-*` variables. Set them on `.combobox-popup` to restyle one Multi Select, or on `:root` for all of them.
