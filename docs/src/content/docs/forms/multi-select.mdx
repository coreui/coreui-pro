...
<Example pro code={`<select id="ms1" data-coreui-multi-select multiple data-coreui-search="global">
...
<Example pro code={`<select data-coreui-multi-select multiple data-coreui-search="global">
    <option value="0">Angular</option>
    <option value="1">Bootstrap</option>
    <option value="2">React.js</option>
    <option value="3">Vue.js</option>
...
The focus never enters the options list: it stays on the toggler, or on the search input when `search` is on, and `aria-activedescendant` points at the highlighted option. The select all button sits in the panel header outside the list, so <kbd>Tab</kbd> reaches it.

### Keyboard shortcuts

| Key | Action |
| --- | --- |
| <kbd>Tab</kbd> | Navigate to/from the component, and to the select all button |
| <kbd>Enter</kbd>/<kbd>Space</kbd> | Open dropdown or select the highlighted option |
| <kbd>Arrow Up</kbd>/<kbd>Arrow Down</kbd> | Move the highlight through the options (and the selectable group labels) |
| <kbd>Home</kbd> | Highlight the first option (not while typing in the search input) |
| <kbd>End</kbd> | Highlight the last option (not while typing in the search input) |
