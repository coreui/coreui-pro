...
With the default `optionsStyle: 'checkbox'` the button shows the same tri-state indicator as the options and mirrors the overall selection — `none` when nothing is selected, `all` when everything is, and `indeterminate` in between; `aria-pressed` carries the same three states. With `optionsStyle: 'text'` the button is a plain text toggle. It sits in the panel header, outside the options list, so <kbd>Tab</kbd> reaches it — the arrows stay on the options.
...
| `optionsStyle` | string | `'checkbox'` | Sets the indicator used by the options, the selectable group labels and the select all button: `'checkbox'` or `'text'`. |
