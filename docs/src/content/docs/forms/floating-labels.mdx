...
When using `.input-group` and `.form-floating` along with form validation, wrap the group in a [`.form-field`](/forms/field/) and place the feedback outside the `.input-group`, so it renders under the whole group rather than inside the frame. Don't add `.has-validation` here — that class exists for feedback sitting *inside* the group as its last child, and with the feedback outside it would leave the addons fully rounded:
