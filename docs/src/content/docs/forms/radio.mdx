...
The v5 spelling — `.form-check` wrapping a `.form-check-input` and a `.form-check-label` — still works and renders exactly as it did. It ships in the opt-in [`coreui-legacy.css`](/getting-started/install/#v5-compatibility-css) rather than in `coreui.css`, and is removed in v7.
...
<ScssDocs file="scss/mixins/_radio.scss" capture="radio-css-vars" />
...
<ScssDocs file="scss/mixins/_radio.scss" capture="radio-variables" />

### Sass maps

Each size is one entry, and the map merges with your overrides — add a key to define a size, or set one to `null` to drop it:

<ScssDocs file="scss/mixins/_radio.scss" capture="radio-sizes" />
