...
The toggle button carries the panel's state — `aria-haspopup="dialog"`, `aria-controls` pointing at the popup, and `aria-expanded` tracking whether it is open — so a screen reader announces what the button does and whether the panel is showing. The panel itself is a `dialog` with `aria-modal`, so focus moves into it on open, stays there while it is open, and returns to the field when it closes.
