...
Autocomplete has no variables of its own: the field is a [`.form-control-group`](/forms/form-control-group/) and takes the `--cui-control-*` properties, and the options dropdown is the shared combobox surface with its own `--cui-combobox-*` ones.
