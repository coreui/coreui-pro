...
import autocompleteDisabledOptions from './snippets/autocomplete-disabled-options.js?raw'
...
<Example pro code={`<div id="myAutoCompleteDisabled"></div>`} sandboxJs={autocompleteDisabledOptions} />

<Code code={autocompleteDisabledOptions} lang="js" />
...
<Example pro code={`<div data-coreui-toggle="autocomplete" data-coreui-disabled="true" data-coreui-placeholder="Disabled autocomplete..." ></div>`} />
...
| `options` | array, string, null | `[]` | Array of options or option objects to populate the dropdown. An option object takes `label`, `value`, `disabled` and `selected`, and a group takes `label` plus a nested `options` array. A string is read as a comma-separated list of labels, which is what `data-coreui-options="Angular, React, Vue.js"` relies on. |
