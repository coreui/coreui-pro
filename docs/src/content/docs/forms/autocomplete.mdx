...
<Example pro code={`<div data-coreui-autocomplete data-coreui-options="Angular, Bootstrap, React.js, Vue.js" data-coreui-placeholder="Search technologies..."></div>`} />

### Global search

Enable global search functionality that works across the entire component, regardless of focus position:

<Example pro code={`<div data-coreui-autocomplete data-coreui-search="global" data-coreui-options="Angular, Bootstrap, React.js, Vue.js" data-coreui-placeholder="Global search enabled..."></div>`} />
...
<Example pro code={`<div data-coreui-autocomplete data-coreui-allow-only-defined-options="true" data-coreui-options="Angular, Bootstrap, React.js, Vue.js" data-coreui-placeholder="Only predefined options allowed..."></div>`} />
...
<Example pro code={`<div data-coreui-autocomplete data-coreui-options="Angular, Bootstrap, React.js, Vue.js" data-coreui-placeholder="Type to see hints..." data-coreui-show-hints="true"></div>`} />

### Highlight matching text

Highlight matching portions of suggestions during search:

<Example pro code={`<div data-coreui-autocomplete data-coreui-highlight-options-on-search="true" data-coreui-options="Angular, Bootstrap, React.js, Vue.js" data-coreui-placeholder="Search with highlighting..."></div>`} />

## Validation states

Apply validation styling to indicate input validity.

<Example pro code={`<div data-coreui-autocomplete data-coreui-options="Angular, Bootstrap, React.js, Vue.js" data-coreui-placeholder="Valid autocomplete..." data-coreui-valid="true"></div>`} />

<Example pro code={`<div data-coreui-autocomplete data-coreui-options="Angular, Bootstrap, React.js, Vue.js" data-coreui-placeholder="Invalid autocomplete..." data-coreui-invalid="true"></div>`} />

## Disabled

Add the `data-coreui-disabled="true"` attribute to disable the component:

<Example pro code={`<div data-coreui-autocomplete data-coreui-disabled="true" data-coreui-options="" data-coreui-placeholder="Disabled autocomplete..." ></div>`} />
...
<Example pro code={`<div data-coreui-autocomplete data-coreui-cleaner="true" data-coreui-options="Angular, Bootstrap, React.js, Vue.js" data-coreui-placeholder="With cleaner button..."></div>`} />
...
Add the `data-coreui-autocomplete` attribute to a container element:

```html
<div data-coreui-autocomplete data-coreui-name="framework" data-coreui-options="JavaScript, TypeScript, React, Vue.js, Angular" data-coreui-search="global"></div>
```
...
```js
const autoCompleteElement = document.getElementById('myAutocomplete')
const autoComplete = new coreui.Autocomplete(autoCompleteElement, {
  name: 'framework',
  options: [
    'JavaScript',
    'TypeScript',
...
| `ariaPickerLabel` | string | `'Toggle options list'` | Accessible label for the picker button, read by screen readers. |
...
| `container` | string, element, boolean | `false` | Appends the dropdown to a specific element. Example: `container: 'body'`. Inside an open dialog it goes into the dialog instead, because the rest of the page is inert while a modal dialog is open. |
...
| `name` | string, null | `null` | Name of the text input the form posts. That input holds what the user sees — the selected option's label, or their own text when `allowOnlyDefinedOptions` is off — so `{ value: 'ng', label: 'Angular' }` posts `Angular`, not `ng`. Without a name the autocomplete is not submitted with the form. |
| `options` | array, string, null | `[]` | Array of options or option objects to populate the dropdown. |
...
| `pickerIcon` | string, boolean | `false` | The button that opens the options menu: `true` for the built-in icon, an inline SVG string for your own (sanitized), `false` for no button. |
...
| `searchNoResultsLabel` | boolean, string | `false` | Text displayed when no search results are found. |
...
| `dispose` | Destroys the instance and removes the input, buttons and panel it created, along with the classes it put on the host. |
...
| Event | Description |
| --- | --- |
| `change.coreui.autocomplete` | Fires when an option is selected or the input value changes. |
...
myAutocomplete.addEventListener('change.coreui.autocomplete', event => {
...
- **Focus management**: The focus never leaves the input — `aria-activedescendant` points at the highlighted option, so typing and choosing happen in one place. <kbd>Home</kbd>, <kbd>End</kbd> and the space bar stay with the text.
...
| Key | Action |
| --- | --- |
| <kbd>Arrow Down</kbd> | Highlight the next option or open dropdown |
| <kbd>Arrow Up</kbd> | Highlight the previous option |
...
Autocomplete has no variables of its own: the field is a [`.form-control-group`](/forms/form-control-group/) and takes the `--cui-control-*` properties, while the options panel is the shared combobox surface, which only sizes itself.

<ScssDocs file="scss/_combobox.scss" capture="combobox-tokens" />

The options themselves are a [List box](/components/list-box/), so their padding, colors, indicator and section labels come from its `--cui-list-box-*` variables — set them on `.combobox-popup` to restyle one Autocomplete, or on `:root` for all of them.
