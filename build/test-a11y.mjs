 ...
 * build/test-a11y.mjs). Deviations: the criterion catalog lives in
 * build/wcag.mjs (plain .mjs, importable under the CI Node version), the docs
 * root is docs/src/content/docs, and the dist bundle is UMD so it loads via a
 * plain file:// <script src> with no inlining workaround.
 *
 * Driven by the unified config in build/a11y.config.mjs (which WCAG criteria
 * each component is tested against). For every configured component it renders
 * that component's documented `<Example>` snippets in headless Chromium (with
 * the compiled CSS + JS bundle) and runs axe-core, scoped to the axe rules
 * mapped to each declared criterion (see `wcagAxeRules` in build/wcag.mjs).
 *
...
import {
  wcagCriteria, a11yStatuses, a11yStatusLabels, wcagAxeRules
} from './wcag.mjs'

const require = createRequire(import.meta.url)
const rootDir = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..')
const docsDir = path.join(rootDir, 'docs/src/content/docs')
const axePath = require.resolve('axe-core')
const cssHref = pathToFileURL(path.join(rootDir, 'dist/css/coreui.css')).href
const jsHref = pathToFileURL(path.join(rootDir, 'dist/js/coreui.bundle.js')).href
  ...
  for (const [index, entry] of entries.entries()) {
    ...
    for (const item of entry.criteria) {
      if (!wcagCriteria[item.criterion]) {
        errors.push(`${where}: unknown criterion "${item.criterion}" (not in wcag.mjs catalog)`)
      }
...
const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'coreui-a11y-'))
...
async function evaluateComponent(page, component, ruleIds) {
  ...
  await page.waitForTimeout(200) // let data-coreui-* plugins auto-initialise
