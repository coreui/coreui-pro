 ...
 * site/src/data/wcag.ts) — kept as plain .mjs because the test runner imports
 * it under the CI Node version, and the docs can consume .mjs just as well.
 *
 * The canonical metadata for each criterion (title, level, version, and the
 * "Understanding" URL) is derived from the W3C WCAG 2.2 specification. The
 * `summary` field is a terse one-liner of our own, not the spec's verbose
 * "Understanding" prose.
 */

/**
 * @typedef {'A' | 'AA' | 'AAA'} WcagLevel
 * @typedef {object} WcagCriterion
 * @property {string} title Human-readable criterion name, e.g. "Identify Input Purpose".
 * @property {WcagLevel} level Conformance level.
 * @property {'2.0' | '2.1' | '2.2'} version WCAG version the criterion was introduced in.
 * @property {string} url Link to the W3C "Understanding" page.
 * @property {string} summary Our terse gloss of what the criterion requires.
 */

/** @type {Record<string, WcagCriterion>} */
export const wcagCriteria = {
  ...
  '2.2.2': {
    ...
    summary: 'Moving, blinking, or auto-updating content can be paused, stopped, or hidden by the user.'
  },
  '2.4.3': {
    title: 'Focus Order',
    level: 'A',
...
export const wcagCriterionIds = Object.keys(wcagCriteria)

/** Status of a criterion for a given component. */
export const a11yStatuses = ['built-in', 'partial', 'author']
...
export const a11yStatusDescriptions = {
